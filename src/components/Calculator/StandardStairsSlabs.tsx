...
const StandardStairsSlabs: React.FC<StandardStairsSlabsProps> = ({ stairResult }) => {
  ...
  const calculateSlabs = () => {
    ...
    for (let i = 0; i < stairResult.totalSteps; i++) {
      ...
      const totalWidth = stairResult.totalWidth || 
                         parseFloat(stairResult.materials[0]?.courseDetails?.find(c => c.step === i + 1)?.width) || 
                         parseFloat(stairResult.materials[0]?.width) || 120; // Default to 120 if not available
      
      // Use side overhang from input
      const sideOverhang = stairResult.sideOverhang || 0;
      console.log('Side Overhang:', sideOverhang);
      const frontWidth = totalWidth - (2 * sideOverhang);
      console.log('Front Width:', frontWidth);
      ...
      const usableWastePieces = wasteList.filter(waste => {
        ...
        const canCoverWidth = waste.width >= frontWidth || waste.length >= frontWidth;
      ...
      if (usableWastePieces.length > 0) {
        ...
        if (selectedWaste.length >= totalWidth || selectedWaste.width >= totalWidth) {
          ...
          if (needsCut(selectedWaste.width, frontWidth)) {
            totalCuts += piecesCount; // Change from totalCuts += 1
          }
        } else {
      ...
      const usableFrontWastePieces = wasteList.filter(waste => {
        ...
        const fitsNormal = waste.width >= stepDimension.height;
        
        // Check if waste fits when rotated (only need to cover height)
        const fitsRotated = waste.length >= stepDimension.height;
      ...
      if (usableFrontWastePieces.length > 0) {
        ...
        // Calculate how many slabs we need without considering gaps
        const slabsNeededWithoutGaps = Math.ceil(frontWidth / slabWidth);
        ...
        if (selectedCutting === 'oneCut') {
          ...
          smallerPieceWidth = frontWidth - widthCoveredByFullSlabs - totalGaps;
        ...
        if (mainPieceCount > 0 && smallerPieceCount > 0) {
          frontSlabDimensions = `${mainPieceCount}x(${mainPieceWidth.toFixed(1)}x${stepDimension.height.toFixed(1)}cm) + ${smallerPieceCount}x(${smallerPieceWidth.toFixed(1)}x${stepDimension.height.toFixed(1)}cm) [Using waste from ${selectedWaste.source}]`;
        } else if (mainPieceCount > 0) {
          frontSlabDimensions = `${mainPieceCount}x(${mainPieceWidth.toFixed(1)}x${stepDimension.height.toFixed(1)}cm) [Using waste from ${selectedWaste.source}]`;
        } else if (smallerPieceCount > 0) {
          frontSlabDimensions = `${smallerPieceCount}x(${smallerPieceWidth.toFixed(1)}x${stepDimension.height.toFixed(1)}cm) [Using waste from ${selectedWaste.source}]`;
        }
        ...
        if (needsCut(selectedWaste.length, stepDimension.height)) {
        ...
        if (totalWidth < (slabsNeededWithoutGaps * slabWidth) + totalGaps) {
          ...
          if (selectedCutting === 'oneCut') {
            ...
            const adjustedFirstSlabWidth = slabWidth - sideOverhang;
            const adjustedLastSlabWidth = remainingWidth - sideOverhang;
            
            frontSlabsNeeded = fullSlabsCount + 1;
            frontNewSlabsNeeded = frontSlabsNeeded; // All new slabs needed
            frontCutLength = adjustedLastSlabWidth;
            totalCuts += 1;
            ...
            if (frontCutLength < slabWidth) {
              ...
              wasteList.push({
                width: slabWidth,
                length: slabLength - stepDimension.height,
                source: `Step ${i+1}`,
                canBeRotated: true
              });
              // Second slab waste if we used two slabs
            ...
            // Calculate dimensions for each slab
            if (fullSlabsCount > 0) {
              // Adjust the description to show individual slab dimensions with counts
              frontSlabDimensions = `${fullSlabsCount}x(${adjustedFirstSlabWidth.toFixed(1)}x${stepDimension.height.toFixed(1)}cm) + 1x(${Math.max(0, adjustedLastSlabWidth).toFixed(1)}x${stepDimension.height.toFixed(1)}cm)`;
            } else {
              frontSlabDimensions = `1x(${Math.max(0, adjustedLastSlabWidth).toFixed(1)}x${stepDimension.height.toFixed(1)}cm)`;
            }
            
            // Count cuts with precise threshold
            if (needsCut(slabLength, stepDimension.height)) {
              totalCuts += fullSlabsCount; // Length cuts for full pieces
              totalCuts += fullSlabsCount; // Width cuts for full pieces
              totalCuts += 1; // Length cut for the cut piece
            }
            ...
            // For full slabs, check if width cut is needed
            if (fullSlabsCount > 0 && needsCut(slabWidth, adjustedFirstSlabWidth)) {
              totalCuts += fullSlabsCount; // Width cuts for full slabs if needed
            }
          } else {
            // 2 cuts option: Take one full slab off, add remaining width, divide into 2 equal pieces
            if (slabsNeededWithoutGaps > 1) {
              ...
              if (frontWasteUsed) {
                frontSlabDimensions = `${fullSlabsCount}x(${slabWidth}x${stepDimension.height.toFixed(1)}cm) + 2x(${equalPieceWidth.toFixed(1)}x${stepDimension.height.toFixed(1)}cm) [Using waste from ${frontWasteSource}]`;
              } else {
                frontSlabDimensions = `${fullSlabsCount}x(${slabWidth}x${stepDimension.height.toFixed(1)}cm) + 2x(${equalPieceWidth.toFixed(1)}x${stepDimension.height.toFixed(1)}cm)`;
                frontSlabDimensions = `2x(${equalPieceWidth.toFixed(1)}x${stepDimension.height.toFixed(1)}cm)`;
                frontSlabDimensions = `2x(${equalPieceWidth.toFixed(1)}x${stepDimension.height.toFixed(1)}cm)`;
              }
              ...
              frontSlabDimensions = `1x(${Math.max(0, totalWidth).toFixed(1)}x${stepDimension.height.toFixed(1)}cm)`;
            }
          }
        } else {
          // No cutting needed, all full slabs
          ...
          frontNewSlabsNeeded = frontSlabsNeeded; // All new slabs needed
          frontSlabDimensions = `${slabsNeededWithoutGaps}x(${slabWidth}x${stepDimension.height.toFixed(1)}cm)`;
        }
      ...
      stepResults.push({
        ...
        frontNeedsCutting,
        frontCutLength,
        stepHeight: stepDimension.height,
        frontWasteUsed,
        frontWasteSource,
        totalWidth,
        frontWidth,
            ...
            <table className="w-full table-fixed bg-gray-700 rounded-lg">
              ...
              <tbody>
                {slabCalculationResult.stepResults.map((result, index) => (
                  <>
                    <tr key={`step-${result.step}`} className="border-b border-gray-600">
                    ...
                    <tr key={`front-${result.step}`} className="border-b border-gray-600">
                      <td className="py-2 px-4 text-left text-gray-300">{result.step}</td>
