...
const SlabCalculator: React.FC<SlabCalculatorProps> = ({ onResultsChange }) => {
  ...
  const calculate = async () => {
    ...
    try {
      ...
      if (mainTaskHours > 0) {
        breakdown.push({ task: `${selectedSlabType.name}`, hours: mainTaskHours });
      }
      ...
      if (cuttingHours > 0) {
        const isPorcelain = selectedSlabType.name.toLowerCase().includes('slab') && 
                           !selectedSlabType.name.toLowerCase().includes('sandstone');
        const cuttingTaskName = isPorcelain ? 'Cutting Slabs' : 'Cutting Sandstones';
        breakdown.push({ task: cuttingTaskName, hours: cuttingHours });
      }
