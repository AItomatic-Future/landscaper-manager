...
interface AdditionalTask {
  ...
  additional_task_materials?: AdditionalTaskMaterial[];
...
const AdditionalFeatures: React.FC<Props> = ({ eventId }) => {
  ...
  const [taskDetails, setTaskDetails] = useState<Omit<AdditionalTask, 'id' | 'progress' | 'hours_spent' | 'is_finished'>>({
    description: '',
  ...
  const { data: additionalTasks = [] } = useQuery({
    queryKey: ['additional_tasks', eventId],
    queryFn: async () => {
      ...
      return (data || []).map(task => {
        ...
        const totalProgress = progressEntries.reduce((sum, entry) => sum + (entry.progress_percentage || 0), 0);
        ...
        const materials = (task.materials_delivered || []).map(m => ({
  ...
  const { data: additionalMaterials = [] } = useQuery({
    queryKey: ['additional_materials', eventId],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const addTaskMutation = useMutation({
    mutationFn: async () => {
      ...
      // Insert materials to materials_delivered if they exist
          ...
          const { error: materialsError } = await supabase
            ...
            .insert(
              validMaterials.map(material => ({
                ...
                amount: parseFloat(material.quantity.toString()),
                unit: material.unit,
                total_amount: parseFloat(material.quantity.toString()),
                status: 'pending'
              }))
            );
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['materials_delivered'] });
  ...
  const addMaterialMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['additional_materials', eventId] });
  ...
  const handleMaterialTemplateChange = (templateId: string) => {
    ...
    if (templateId === 'other') {
      ...
      const template = materialTemplates.find(m => m.id === templateId);
      if (template) {
  ...
  const addProgressMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error: entryError } = await supabase
        ...
        .insert({
          ...
          progress_percentage: progressDetails.progress,
          hours_spent: progressDetails.hoursWorked,
    ...
    onSuccess: () => {
      ...
      setProgressDetails({ progress: 0, hoursWorked: 0, notes: '' });
    },
    onError: (error) => {
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setProgressDetails(prev => ({
                  ...prev,
                  hoursWorked: Math.max(0, parseFloat(e.target.value) || 0)
                }))}
                min="0"
                step="0.5"
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
