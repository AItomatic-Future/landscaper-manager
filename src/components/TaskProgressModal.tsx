...
const TaskProgressModal: React.FC<TaskProgressModalProps> = ({ task, onClose }) => {
  ...
  const { data: progressEntries = [] } = useQuery({
    queryKey: ['task_progress', task?.id],
    queryFn: async () => {
      if (!task?.id) return [];
  ...
  const addProgressEntryMutation = useMutation({
    mutationFn: async ({ taskId, amount, hours }: { taskId: string; amount: number; hours: number }) => {
      ...
      // Get the event_tasks_id for this task
      ...
      // Find the corresponding event_tasks record
      const { data: eventTaskData, error: eventTaskError } = await supabase
        .from('event_tasks')
        .select('id')
        .eq('name', taskData.name)
        .single();
        
      if (eventTaskError && eventTaskError.code !== 'PGRST116') {
        // PGRST116 is "Results contain 0 rows" - we'll handle this case by not setting event_tasks_id
        console.error('Error fetching event_tasks:', eventTaskError);
        throw eventTaskError;
      }
      
      const eventTasksId = eventTaskData?.id || null;
      ...
      <div className="space-y-4">
        ...
        <div className="bg-gray-50 p-4 rounded-lg space-y-2">
          <p className="text-sm text-gray-600">
            Total Completed: <span className="font-medium">{totalCompleted} {unit}</span>
          </p>
          <p className="text-sm text-gray-600">
            Remaining: <span className="font-medium">{remainingAmount} {unit}</span>
          </p>
          <p className="text-sm text-gray-600">
            Total Hours Spent: <span className="font-medium">{totalHoursSpent}</span>
          </p>
          <p className="text-sm text-gray-600">
            Remaining Hours: <span className="font-medium">{remainingHours}</span>
          </p>
        </div>
