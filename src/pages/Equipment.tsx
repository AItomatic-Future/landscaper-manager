...
const EquipmentPage = () => {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status, usage }: { id: string; status: string; usage?: EquipmentUsage }) => {
      if (status === 'in_use' && usage) {
        ...
        // Check if there are any in-use quantities
        const { data: currentEquipment } = await supabase
          .from('equipment')
          .select('in_use_quantity')
          .eq('id', id)
          .single();
          
        if (!currentEquipment) throw new Error('Equipment not found');
        
        // Only update to free_to_use if no quantities are in use
  ...
  const handleStatusUpdate = () => {
    ...
    if (newStatus === 'in_use') {
      ...
      // For free_to_use or broken status
      updateStatusMutation.mutate({
        id: selectedEquipment.id,
        status: newStatus
      });
              ...
              <div className="flex-1">
                ...
                <div className="mt-2 space-y-2">
                  <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(item.status, item)}`}>
                    {item.quantity > item.in_use_quantity ? 'free to use' : item.status.replace(/_/g, ' ')}
                  </span>
                  <div className="text-sm text-gray-600">
                    Quantity: {item.in_use_quantity}/{item.quantity} in use
                  </div>
                </div>
              </div>
