...
import { Loader2, X, PenTool as Tool, Package, Pencil, Info } from 'lucide-react';
...
const TaskRequirements = () => {
  ...
  const addTaskMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['task_requirements'] });
                  ...
                  <ul className="space-y-2">
                    {selectedTask.tools.map((tool, index) => (
                      <li key={index} className="flex items-center text-blue-800">
                        <Tool className="w-4 h-4 mr-2 flex-shrink-0" />
